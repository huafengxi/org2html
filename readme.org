...
* sync
This repo is synced across machines via the dev mirror (origin=dev:~/git) with
an event-driven fast-forward push-back hook to GitHub, plus periodic ff-only
auto-pulls on each machine (see ~/m SETUP.md "代码自动同步").
